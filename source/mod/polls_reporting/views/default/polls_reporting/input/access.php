<?php
	...
	if (!array_key_exists('value', $vars) || $vars['value'] == ACCESS_DEFAULT)
		...
		if ((!isset($vars['options'])) || (!is_array($vars['options'])))
		{
			...
			foreach($vars['options'] as $option_id => $option_value)
			{
				if($option_id < 0)
					unset($vars['options'][$option_id]);
			}
		}
...
<select name="<?php echo $vars['internalname']; ?>"<?php echo $id; ?> <?php if (isset($vars['js'])) echo $vars['js']; ?> <?php if ((isset($vars['disabled'])) && ($vars['disabled'])) echo ' disabled="yes" '; ?> class="<?php echo $class; ?>">
<?php
